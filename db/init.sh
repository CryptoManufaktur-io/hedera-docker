...
if [[ -n "$SKIP_SNAPSHOT_INIT" && "$SKIP_SNAPSHOT_INIT" == "false" ]]; then
    echo "Database will be setup by snapshot init since SKIP_SNAPSHOT_INIT=false"
    exit 0
else
    echo "Setting up the database since SKIP_SNAPSHOT_INIT=true"
fi
